...
export function ActivityTimeline({
  ...
  const getEventIcon = (title: string, index: number) => {
    ...
    if (title.toLowerCase().includes('analytic')) {
      return <TextSearch className="h-4 w-4 text-neutral-400" />;
    } else if (title.toLowerCase().includes('rag')) {
      return <FileStack className="h-4 w-4 text-neutral-400" />;
    } else if (title.toLowerCase().includes('formatting')) {
      return <LetterText className="h-4 w-4 text-neutral-400" />;
    }
